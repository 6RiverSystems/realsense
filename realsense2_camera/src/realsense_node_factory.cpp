...
void RealSenseNodeFactory::notification_handler(const rs2::notification &n, int iteration)
{
    ...
    std::thread([this]() {
        ROS_INFO_STREAM("Notification: executing reset for device " << _usb_port_id);
        ROS_INFO_STREAM("Notification: device " << _usb_port_id << " sleeping for 1 second");
        boost::this_thread::sleep(boost::posix_time::seconds(1));
        ROS_INFO_STREAM("Notification: device " << _usb_port_id << " stoping topics");
        try
        {
        ...
        ROS_INFO_STREAM("Notification: device " << _usb_port_id << " deallocating realsensenode");
        try
        {
        ...
        ROS_INFO_STREAM("Notification: device " << _usb_port_id << " sleeping for 1 second");
        boost::this_thread::sleep(boost::posix_time::seconds(1));
        ROS_INFO_STREAM("Notification: device " << _usb_port_id << " creating a new device");
        try
        {
            _device = rs2::device();
        }
        ...
        while (!found)
        {
            for (auto &&dev : _context.query_devices())
            {
                ...
                {
                    ROS_INFO_STREAM("Notification: device " << _usb_port_id << " resetting");
                    try
                    {
                        dev.hardware_reset();
                    }
                    ...
                    ROS_INFO_STREAM("Notification: device " << _usb_port_id << " found... and was reset");
                    found = true;
                    break;
                }
            }
        ...
        ROS_INFO_STREAM("Notification: device " << _usb_port_id << " sleeping for 10 seconds");
        ...
        while (!found)
        {
            for (auto &&dev : _context.query_devices())
            {
                if (deviceMatches(dev, _usb_port_id))
                {
                    ...
                    ROS_INFO_STREAM("Notification: device " << _usb_port_id << " found... and was added");
                    found = true;
                    break;
                }
            }
...
void RealSenseNodeFactory::setUpResinChuck()
{
...

    {
        ...
        while (!found)
        {
            ...
            if (!found)
            {
                ROS_ERROR_STREAM("Notification: oo devices found for adding... " << _usb_port_id << " sleeping for 2 seconds and polling again");
                boost::this_thread::sleep(boost::posix_time::seconds(2));
                _context = rs2::context{};
            }
        }
...
void RealSenseNodeFactory::addDevice(rs2::device dev)
{
    ...
    _handler =
            [this, local_copy_of_iteration](const rs2::notification &n) {
                ROS_INFO_STREAM("Notification: callback for device " << _usb_port_id
